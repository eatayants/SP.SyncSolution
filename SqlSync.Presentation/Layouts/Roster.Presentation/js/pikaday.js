...
(function (root, factory) {
    ...
    renderTimePicker = function (num_options, step, selected_val, select_class, display_func) {
        var to_return = '<td><select class="pika-select ' + select_class + '">';
        for (var i = 0; i < num_options; (i += step)) {
    ...
    renderTime = function (hh, mm, ss, opts) {
        var to_return = '<table cellpadding="0" cellspacing="0" class="pika-time"><tbody><tr>' +
            ...
            '<td>:</td>' +
            renderTimePicker(60, 5, mm, 'pika-select-minute', function (i) { if (i == 0 || i == 5) return "0" + i; return i }) +
            '<td><button class="pika-button is-empty pika-submit" type="button">ok</button></td>';

        if (opts.showSeconds) {
            to_return += '<td>:</td>' +
                renderTimePicker(60, 1, ss, 'pika-select-second', function (i) { if (i < 10) return "0" + i; return i });
        }
        ...
        self._onMouseDown = function (e) {
            ...
            if (!hasClass(target.parentNode, 'is-disabled')) {
                if (hasClass(target, 'pika-button') && !hasClass(target, 'is-empty')) {
                    ...
                    self.setDate(newDate, opts.showTime);
                    if (opts.bound) {
                        sto(function () {
                            self.hide();
                            if (opts.field) {
                ...
                else if (hasClass(target, 'pika-submit')) {
                    self.setDate(self.getDate(), false); // close dialog
                }
            }
